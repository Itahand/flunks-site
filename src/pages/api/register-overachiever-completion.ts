...
/**
 * Register Overachiever Completion API
 * 
 * Calls blockchain Admin function to register overachiever completion
 * (All weekly objectives complete)
 */
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Register overachiever completion on blockchain
    ...
    console.log(`✅ Overachiever completion registered for ${walletAddress}: ${transactionId}`);
    ...
    return res.status(200).json({
      success: true,
      transactionId,
      message: 'Overachiever completion registered on blockchain',
    });
  } catch (error: any) {
    console.error('Error registering overachiever completion:', error);
    return res.status(500).json({
      error: 'Failed to register overachiever completion',
