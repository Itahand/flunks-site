-- FIX: Drop duplicate award_gum functions and recreate a single clean version
-- The issue: Two versions exist with different parameter types (VARCHAR vs TEXT)
-- causing "Could not choose the best candidate function" errors

-- First, drop ALL versions of the function
DROP FUNCTION IF EXISTS award_gum(VARCHAR(64), VARCHAR(64), JSONB);
DROP FUNCTION IF EXISTS award_gum(TEXT, TEXT, JSONB);
DROP FUNCTION IF EXISTS award_gum(VARCHAR, VARCHAR, JSONB);

-- Now create the single, correct version using TEXT (most compatible)
  ...
  ELSE
    ...
    RETURN jsonb_build_object(
      ...
      'cooldown_remaining_minutes', CASE
        WHEN p_source = 'daily_login' OR p_source = 'daily_checkin' THEN 0
